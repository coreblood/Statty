...
-- QoL: taken from the load vararg, NOT a literal. Inside the merged addon
-- ADDON_LOADED reports the FOLDER name ("UncappedQoL"); the old hardcoded
-- "StatFeed" never matched there, so BuildWindow never ran and the feed
-- window silently didn't exist.
local ADDON_NAME   = ... or "StatFeed"
...
local PLAN_STRIP_H = 18       -- QoL: the shares/save strip under the stat rows
...
EXPANDED_MIN = STAT_BLOCK_TOP                           -- top of the stat block
             + (#STATS * STAT_ROW + 4 + PLAN_STRIP_H)   -- the stat block + plan strip
...
local function LayoutStatColumns()
    ...
    -- QoL: when the plan controls are live they own a fixed strip on the
    -- right of every row; every proportional split below works on what is
    -- left. The getter lives in QoLBridge.lua so a missing bridge costs 0px.
    local reserve = (_G.StatFeedQoL_ControlsReserve and _G.StatFeedQoL_ControlsReserve()) or 0
    local width = statBlock:GetWidth() - reserve
    local addedW = 40
    ...
    addedW = min(max(addedW, 40), max(40, floor(width * 0.30)))

    -- Name gets first call on width so "Spell Power" / "Defense Rating" stay on
    -- one line; the start (middle) column takes what's left and can compress.
    local nameW = max(78, floor(width * 0.42))
    local startW = max(34, width - nameW - addedW - 12)
    ...
    for i = 1, #STATS do
        ...
        if row then
            ...
            if row.y then row.added:SetPoint("TOPRIGHT", -(2 + reserve), row.y) end
        end
    end
end
...
local function HandleFeed(message)
    ...
    -- Auto-convert line arrives on THIS feed channel (DSTATS), not
    -- CHAT_MSG_SYSTEM. Hand the numbers to Core, which prints the per-stat
    -- breakdown and updates the gained/lost tooltip counters.
    local spent, gained = match(clean, "spent (%d[%d,]*) to gain (%d[%d,]*)")
    if spent and _G.StattyQoL_RecordConvert then
        _G.StattyQoL_RecordConvert((gsub(spent, ",", "")), (gsub(gained, ",", "")))
    end
end
...
local function BuildWindow()
    ...
    statBlock:SetHeight(#STATS * STAT_ROW + 4 + PLAN_STRIP_H)

    for i = 1, #STATS do
        local info = STATS[i]
        local y    = -((i - 1) * STAT_ROW) - 2
        local row  = {}
        row.y      = y -- QoL: the controls reserve re-anchors row.added at this y
        ...
        row.name:SetHeight(STAT_ROW - 2)   -- one line only; no vertical wrap
        ...
        row.hit:SetScript("OnEnter", function(self)
            ...
            -- QoL: how much of this stat came FROM auto-conversion this session.
            local gained = _G.StattyQoL_ConvGained and _G.StattyQoL_ConvGained(s.key) or 0
            if gained > 0 then
                GameTooltip:AddLine("Gained by conversion: +" .. Commas(gained), 0.4, 0.9, 0.4)
            end
            local lost = _G.StattyQoL_ConvLost and _G.StattyQoL_ConvLost(s.key) or 0
            if lost > 0 then
                GameTooltip:AddLine("Lost to conversion: -" .. Commas(lost), 1, 0.5, 0.4)
            end
            GameTooltip:Show()
        end)
    ...
    -- QoL: the bridge (QoLBridge.lua, loaded after this file) attaches the
    -- green/red/% plan controls to these rows and builds the strip in the
    -- PLAN_STRIP_H band reserved at the bottom of the block. Passing
    -- LayoutStatColumns lets it relayout when the reserve changes.
    if _G.StatFeedQoL_OnRowsBuilt then
        _G.StatFeedQoL_OnRowsBuilt(statBlock, rows, STATS, STAT_ROW, PLAN_STRIP_H, LayoutStatColumns)
    end
...
events:SetScript("OnEvent", function(self, evt, a1, a2)
    ...
    if e == "ADDON_LOADED" then
        if p1 == ADDON_NAME then
            ...
            StartStartupDelay()
            BuildWindow()
            -- QoL: the UncappedUI page is superseded by the addon's own
            -- "Uncapped QoL > Stat Feed" panel (QoLBridge.lua); registering
            -- both would show the same settings in two places.
            -- BuildOptions()
...
-- QoL: handles for the addon's own options panel (built in QoLBridge.lua).
-- All closures, so the panel never reaches into this file's locals directly.
_G.StattyQoL_SessionEarned = function(statKey) return earned[statKey] or 0 end
_G.StatFeedQoL_API = {
    GetDB = function() return GetDB() end,
    Toggle = function() Toggle() end,
    IsShown = function() return frame and frame:IsShown() or false end,
    SetShown = function(v)
        local db = GetDB()
        db.shown = v and true or false
        if frame then
            if v then frame:Show(); dirty = true; Repaint(true) else frame:Hide() end
        end
    end,
    ApplyStatVisibility = function() ApplyStatVisibility() end,
    ApplyBgAlpha = function()
        if frame then frame:SetBackdropColor(0, 0, 0, GetDB().bgAlpha) end
    end,
    ApplyMaxLines = function()
        if log then log:SetMaxLines(GetDB().maxLines) end
    end,
    ResetWindow = function() ResetWindow() end,
    ResetSession = function()
        ResetSession()
        if _G.StattyQoL_ResetConvGained then _G.StattyQoL_ResetConvGained() end
        AddLine("|cff9CC243Session counters reset.|r")
        Repaint(true)
    end,
    ClearFeed = function() if log then log:Clear() end end,
    AddRawLine = function(msg) AddLine(msg) end,
    -- Per-stat stats earned this session (display name -> amount). Core uses
    -- this to split the pooled "spent" figure across feeders realistically.
    SessionEarned = function(statKey) return earned[statKey] or 0 end,
}
